...
while True:
    ...
    if os.path.isfile('stocks3-1day.h5'):
        print("Loading model...\n")
        model = load_model('stocks3-1day.h5')
        plot_model(model, to_file='stock3-1day.png')
    else:
        print("Building model...\n")
        model = bm.build_model(3, data_column_count, data_row_count, n_classes)
        
    for i in range(0, n_batches):
        print("Loading batch #" + str(i+1) + " of " + str(n_batches))
        x_t = np.load('TrainingData1day/x_train-' + str(i) + '.npy')
        y_t = np.load('TrainingData1day/y_train-' + str(i) + '.npy')
        batches[i] = x_t, y_t
...
#        status = model.fit(x_train, y_train, epochs=1, batch_size=batch_size)
        ...
        model.save("stocks3-1day(index=3; epoch=" + str(q+1) + "; acc=" + str(acc) + ").h5", 
                   overwrite=True)
        ...
        if acc >= 0.85: 
            if acc > best:
                print("Saving model...\n")
                model.save("stocks3-1day.h5", overwrite=True)
                best = acc
            if acc >= 0.98: break
